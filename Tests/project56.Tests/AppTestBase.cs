...
using project56.Tests.TestDatas;
...
namespace project56.Tests
{
    ...
    /// Provides methods to easily work with <see cref="project56DbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<project56TestModule>
    {
        ...
        protected void UsingDbContext(Action<project56DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<project56DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<project56DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<project56DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<project56DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<project56DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<project56DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<project56DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<project56DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<project56DbContext, Task<T>> func)
        {
            T result;
